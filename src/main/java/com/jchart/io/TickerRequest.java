...
public class TickerRequest implements Runnable {
	...
	public TickerRequest(JchartComposite jchartComposite, TickerListener tickerListener) {
	...
	public void requestCompare(String baseTicker, String compareTicker) {										
		...
		if (_tickerFound) {
			...
			else if (!compareTicker.equalsIgnoreCase(_quoteDataModel.getCompareTicker()))
				request(compareTicker, true);
		}
	}
	...
	private void tickerRequest() {
		try {
			boolean isIntraday = _jchartComposite.getTimeFrameModel().isIntraday();
			if (isIntraday)
				_tickerFound = IoFactory.getInstance(_ticker).tickerRequestIntraday(_ticker, _jchartComposite);
			else {
				_tickerFound = IoFactory.getInstance(_ticker).tickerRequestEod(_ticker, _jchartComposite);
			}
